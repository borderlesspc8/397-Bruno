...
if (vendasData.data && Array.isArray(vendasData.data)) {
  vendasData.data.forEach((venda, index) => {
    ...
    if (venda.itens && Array.isArray(venda.itens)) {
      console.log('Itens encontrados:', venda.itens.length);
      
      venda.itens.forEach((item, itemIndex) => {
        ...
        if (nomeProduto) {
          if (produtosMap.has(nomeProduto)) {
            ...
            produto.quantidade += parseInt(item.quantidade) || 0;
      ...
      console.log('Nenhum item encontrado nesta venda');
    }
  });
}
