...
export function BudgetSummary() {
    ...
    try {
      ...
      if (suggestionsData.suggestions && suggestionsData.suggestions.length > 0) {
        ...
        const formattedSuggestions = suggestionsData.suggestions.map((suggestion: any) => ({
          ...
          colorAccent: getColorForCategory(suggestion.category),
        }));
  ...
  function generateBasicSuggestions() {
    ...
    const commonBudgetCategories = [
      { category: "Alimentação", percentage: 25, colorAccent: "#ef4444" },
      { category: "Moradia", percentage: 35, colorAccent: "#f97316" },
      { category: "Transporte", percentage: 15, colorAccent: "#eab308" },
      { category: "Lazer", percentage: 10, colorAccent: "#84cc16" },
      { category: "Saúde", percentage: 10, colorAccent: "#10b981" },
      { category: "Outros", percentage: 5, colorAccent: "#94a3b8" }
    ];
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="space-y-4">
            {isSuggested && (
              <Badge variant="outline" className="mb-2 bg-primary/10 text-primary border-primary/20 inline-flex items-center">
            ...
            <div className="pt-2">
              <Link href={isSuggested ? "/budgets/new?suggested=true" : "/budgets/new"}>
                <Button variant="outline" size="sm" className="w-full">
                  {isSuggested ? "Criar orçamento personalizado" : "Criar orçamento"}
                </Button>
              </Link>
            </div>
          </div>
