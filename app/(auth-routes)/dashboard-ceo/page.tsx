...
export default function DashboardCEO() {
  ...
  useEffect(() => {
    if (user?.id) { // Só logar quando tiver usuário
      console.log("=== DASHBOARD CEO - DEBUG ===");
  ...
  const dadosSummary = useMemo(() => {
    ...
    if (vendas && Array.isArray(vendas)) {
      console.log('🔍 [DashboardCEO] Processando vendas para cálculo financeiro:', {
        totalVendas: vendas.length,
        primeirasVendas: vendas.slice(0, 3).map(v => ({
          id: v.id,
          valor_total: v.valor_total,
      ...
      console.log('💰 [DashboardCEO] Totais calculados:', {
        custoTotal,
        descontosTotal,
        fretesTotal,
        totalValor,
    ...
    <PageContainer>
      <div className="space-y-6 ios26-animate-fade-in">
        ...
        <div className="col-span-12">
          <DashboardHeader 
            title="Dashboard CEO" 
            description="Visão executiva e métricas estratégicas"
            dateRange={dateRange}
