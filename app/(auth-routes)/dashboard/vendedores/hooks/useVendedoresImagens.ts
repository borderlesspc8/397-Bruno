...
/**
 * Hook para gerenciar imagens de vendedores na página de vendedores
 * Segue o mesmo padrão da página de vendas para evitar erros 404
...
export function useVendedoresImagens(vendedores: Vendedor[], limite?: number) {
  ...
  useEffect(() => {
    ...
    vendedoresParaProcessar.forEach(async vendedor => {
      ...
      try {
        const supabaseUrl = await SupabaseStorageService.getImageUrl(id);
        if (supabaseUrl) {
        ...
        console.warn(`Supabase Storage não disponível para vendedor ${id}:`, supabaseError);
      }
