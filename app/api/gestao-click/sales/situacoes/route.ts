...
/**
 * GET /api/gestao-click/sales/situacoes
 * Lista todas as situações de vendas do Gestão Click
 */
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`[DEBUG] GET /api/gestao-click/sales/situacoes - Params: bypass=${bypassAuth}`);
    ...
    try {
      console.log('[DEBUG] Calling GestaoClickClientService.getSituacoesVendas()');
      // Buscar situações de vendas do Gestão Click
      const result = await gestaoClickService.getSituacoesVendas();
      console.log(`[DEBUG] API returned ${result?.data?.length || 0} situações de vendas`);
    ...
    console.error('Erro ao buscar situações de vendas do Gestão Click:', error);
    return NextResponse.json(
      { 
        error: 'Erro ao buscar situações de vendas', 
