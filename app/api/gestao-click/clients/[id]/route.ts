 ...
 * Busca um cliente específico do Gestão Click pelo ID
 */
export async function GET(
  ...
  try {
    ...
    console.log(`[DEBUG] GET /api/gestao-click/clients/${params.id} - Params: bypass=${bypassAuth}`);
    ...
    try {
      console.log(`[DEBUG] Buscando cliente com ID: ${params.id}`);
      // Buscar cliente do Gestão Click
      const cliente = await gestaoClickService.getClienteById(params.id);
      ...
      console.log(`[DEBUG] Cliente encontrado: ${cliente.nome}`);
    ...
    console.error('[ERROR] Error in client detail endpoint:', error);
    return NextResponse.json(
      { 
