...
/**
 * GET /api/gestao-click/clients/[id]/vendas
 * Busca o histórico de vendas de um cliente específico do Gestão Click
 */
export async function GET(
  ...
  try {
    ...
    console.log(`[DEBUG] GET /api/gestao-click/clients/${params.id}/vendas - Params: bypass=${bypassAuth}, page=${page}, limit=${limit}`);
    ...
    try {
      console.log(`[DEBUG] Buscando histórico de vendas para cliente com ID: ${params.id}`);
      
      // Definir filtros para buscar apenas vendas do cliente especificado
      const filtros = {
        cliente_id: parseInt(params.id),
      };
      
      // Buscar vendas do cliente no Gestão Click
      const response = await gestaoClickService.getVendas(filtros, page, limit);
      
      // Verificar se a resposta contém dados ou a mensagem "Não há dados!"
      if (typeof response.data === 'string' && response.data === "Não há dados!") {
        // Converter para o formato esperado com um array vazio
        ...
        console.log(`[DEBUG] Nenhuma venda encontrada para o cliente ${params.id}`);
        return NextResponse.json(formattedResponse);
      }
      
      console.log(`[DEBUG] ${response.data.length} vendas encontradas para o cliente ${params.id}`);
    ...
    console.error('[ERROR] Error in client sales endpoint:', error);
    return NextResponse.json(
      { 
        error: 'Erro ao buscar histórico de vendas do cliente', 
