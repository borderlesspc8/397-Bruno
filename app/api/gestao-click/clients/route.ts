 ...
 * Lista todos os clientes do Gestão Click
 */
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`[DEBUG] GET /api/gestao-click/clients - Params: page=${page}, limit=${limit}, test=${useTestData}, bypass=${bypassAuth}`);
    ...
    try {
      console.log('[DEBUG] Calling GestaoClickClientService.getClientes()');
      // Buscar clientes do Gestão Click
      const result = await gestaoClickService.getClientes(page, limit);
      console.log(`[DEBUG] API returned ${result?.data?.length || 0} clients`);
    ...
    console.error('[ERROR] Error in clients endpoint:', error);
    return NextResponse.json(
      { 
        ...
        timestamp: new Date().toISOString(),
        environment: process.env.NODE_ENV,
        help: process.env.NODE_ENV === 'development' ? 'Para testes em desenvolvimento, use ?bypass=true&test=true' : undefined
      },
      { status: 500 }
    );
  }
