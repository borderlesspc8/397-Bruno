  ...
  try {
    ...
    if (!transactions || !Array.isArray(transactions) || transactions.length === 0) {
      ...
      try {
        ...
        if (userTransactions.length === 0) {
          return NextResponse.json(
            { error: "Sem transações suficientes para gerar sugestões de orçamento" },
            { status: 400 }
          );
        }
    ...
    for (const transaction of body.transactions) {
      if (transaction.type === 'EXPENSE' || !transaction.type) {
        ...
        categorySums.set(
          category, 
          (categorySums.get(category) || 0) + amount
        );
    ...
    const expensesByCategory = Array.from(categorySums.entries()).map(([category, total]) => ({
      category,
      total,
...
`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "Você é um assistente de finanças pessoais especializado em análise de gastos e orçamentos."
        },
        {
          role: "user",
          content: prompt
