 ...
 * 
 * Execução: npm run cleanup-duplicates -- userId=USER_ID
 * 
 * Exemplo: npm run cleanup-duplicates -- userId=clg8zep3g0003gta6h4t7g2kv
 */
...
if (!params.userId) {
  ...
  console.log('Uso: npm run cleanup-duplicates -- userId=USER_ID');
  process.exit(1);
}
...
if (dryRun) {
  ...
  console.log('\x1b[31m%s\x1b[0m', '💡 Para simular sem fazer alterações, use: npm run cleanup-duplicates -- userId=USER_ID dryRun=true');
 ...
 */
async function cleanupDuplicateWallets(): Promise<{
  removidas: number;
  preservadas: number;
  detalhes: any[];
}> {
 ...
 */
async function cleanupDuplicateTransactions(): Promise<{
  removidas: number;
  preservadas: number;
  detalhes: any[];
}> {
